...
void print_jing() {
  ...
  if (GameMode == 0) {
    cout << "当前模式为：普通模式" << endl << endl;
  } else if (GameMode == 1) {
    cout << "当前模式为：仅有3个棋子模式" << endl << endl;
  }
...
void input() {
  while (true) {
    ...
    if (jing[temp] == 0) {
      ...
      cout << temp + 1 << "号位置有" << pring_jing_words(jing[temp]) << "了"
           << endl;
    }
  }
}
